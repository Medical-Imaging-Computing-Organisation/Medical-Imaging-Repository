# -*- coding: utf-8 -*-
"""
Created on Thu Mar 14 14:58:47 2024

@author: alfie

This version of f_t_c is designed to accept MC data with exact positions for
each photon interacion.
"""
...
def find_true_coincidences(tau, epsilon, E0, arrA, arrB, arrA_coeffs, arrB_coeffs, arrA_difference, arrB_difference):
    ...
    THIS IS THE EXACT POSITIONAL VERSION
    ...
    arrA and arrB should be of the format:
        [[Index, E, t, delta_E, delta_t, x, y, z], ...]
    ...
    output: array
        ...
        Format:
            [[E1, E2, delta_E1, delta_E2, scat_index, abs_index, x_scat, y_scat, z_scat, x_abs, y_abs, z_abs],
             [E1, E2, delta_E1, delta_E2, scat_index, abs_index, x_scat, y_scat, z_scat, x_abs, y_abs, z_abs],
             ...]
            ...
            absorption occurred. The x,y,z coordinates of the scatter and
            absorption events are also given.
    ...
    temp_arr = np.ndarray((arrA.shape[0], test_window_size, 12), dtype=np.float32)

    # # define for use later (I had to change dtype to float32 so that my laptop could cope with the memory allocation)
    package = np.empty((arrA.shape[0], test_window_size, 12), dtype=np.float32)
    ...
    column8 = np.empty((arrA.shape[0], test_window_size), dtype=np.float32)
    column9 = np.empty((arrA.shape[0], test_window_size), dtype=np.float32)
    column10 = np.empty((arrA.shape[0], test_window_size), dtype=np.float32)
    column11 = np.empty((arrA.shape[0], test_window_size), dtype=np.float32)
    ...
    def two_array_tester(tau, epsilon, E0, arrA, arrB, temp_arr, arrA_coeffs, arrB_coeffs, arrA_difference, arrB_difference, package, column0, column1, column2, column3, column4, column5):
        ...
        temp_arr : array
            Array of valid coincident scatter-absorption event pairs. Format:
                ...
                [[E1, E2, delta_E1, delta_E2, scatIndex, absIndex, x_scat, y_scat, z_scat, x_abs, y_abs, z_abs],
                 [E1, E2, delta_E1, delta_E2, scatIndex, absIndex. x_scat, y_scat, z_scat, x_abs, y_abs, z_abs],
                 ...]
        ...
        # extract time information from arrA 
        tA = arrA[:, 2]
        delta_tA = arrA[:, 4]
        ...
        # iterate over every event in arrA. For the i^th event:          
            ...
            # x position of scattering detector
            column6[i][:valid_events.shape[0]] = arrA[i, 5]
            column6[i][valid_events.shape[0]:] = 0
            
            # y position of scattering detector
            column7[i][:valid_events.shape[0]] = arrA[i, 6]
            column7[i][valid_events.shape[0]:] = 0
            
            # z position of scattering detector
            column8[i][:valid_events.shape[0]] = arrA[i, 7]
            column8[i][valid_events.shape[0]:] = 0
            
            # x position of absorbing detector
            column9[i][:valid_events.shape[0]] = valid_events[:, 5]
            column9[i][valid_events.shape[0]:] = 0

            # y position of absorbing detector
            column10[i][:valid_events.shape[0]] = valid_events[:, 6]
            column10[i][valid_events.shape[0]:] = 0
            
            # z position of absorbing detector
            column11[i][:valid_events.shape[0]] = valid_events[:, 7]
            column11[i][valid_events.shape[0]:] = 0               
            ...
            package[i, :, 8] = column8[i]
            package[i, :, 9] = column9[i]
            package[i, :, 10] = column10[i]
            package[i, :, 11] = column11[i]

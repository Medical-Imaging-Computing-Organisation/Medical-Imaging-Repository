...
def find_true_coincidences(tau, epsilon, E0, arrA, arrB, arrA_coeffs, arrB_coeffs, arrA_difference, arrB_difference):
    ...
    def two_array_tester(tau, epsilon, E0, arrA, arrB, temp_arr, arrA_coeffs, arrB_coeffs, arrA_difference, arrB_difference, package, column0, column1, column2, column3, column4, column5):
        ...
        # iterate over every event in arrA. For the i^th event:          
            ...
            # also account for the difference between fit timestamp and actual timestamp
            # j = int((tA[i]-arrB_coeffs[1]-arrB_difference[i])/arrB_coeffs[0])
            
            j = int((tA[i]-arrB_coeffs[1])/arrB_coeffs[0])
            
            
            j = int(j - arrB_difference[j]/arrB_coeffs[0])
            
        
            
            # calculate limits of the testing window for the i^th event in arrA
            test_max = int(max(0, min(j + test_window_size, arrB.shape[0])))
            test_min = int(max(0, min(j - 1.5 * test_window_size, arrB.shape[0])))
            
            
            # testing specific value(s) MC gave us
            if i in range(103, 104):
                print("------------------------")
                print(i)
                print("---")
                print(tA[i])
...
if __name__ == "__main__":
    ...
    tau = 0.000001# time coincidence window
    epsilon = 0.001 # energy coincidence window

...
def Generate_Position_Vectors_And_Matrices_CSV2Prime(EArray, DetectorArray, Errors=False):
    '''
    ...
    def Populate_Output_Array(EArray, DetectorArray, vec_mat_arr, angle_arr, unique_index_pairs, Errors = False):
        if Errors == True: # don't expect to ever run this so haven't edited it 14/3/24
            for i in range(0, unique_index_pairs.size):
            
                # Generate OA Vector
                vec_mat_arr[i,:3] = 0.01 * DetectorArray[np.where((DetectorArray[:,0]==EArray[unique_index_pairs[i],4]) & (DetectorArray[:,1]==EArray[unique_index_pairs[i],5])),2:5]
                vec_mat_arr[i, 3:6] = 0
                # Generate BA Vector (Values only)
                vec_mat_arr[i,6:9] = -0.01 *  DetectorArray[np.where((DetectorArray[:,0]==EArray[unique_index_pairs[i],4]) & (DetectorArray[:,1]==EArray[unique_index_pairs[i],5])),8:11] + vec_mat_arr[i,:3]
    ...
    output_arr = Populate_Output_Array(EArray, DetectorArray, vec_mat_arr, angle_arr, unique_index_pairs, Errors)
    return output_arr
